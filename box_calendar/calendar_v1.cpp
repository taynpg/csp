...
void CCalender::initSelf() {
    ...
    m_wuhu[0] = 2; m_wuhu[1] = 4; m_wuhu[2] = 6; m_wuhu[3] = 8; 
    m_wuhu[4] = 0; m_wuhu[5] = 2; m_wuhu[6] = 4; m_wuhu[7] = 6; 
    m_wuhu[8] = 8; m_wuhu[9] = 0; 

    m_qishi[0] = 0; m_qishi[1] = 2; m_qishi[2] = 4; m_qishi[3] = 6;
    m_qishi[4] = 8; m_qishi[5] = 0; m_qishi[6] = 2; m_qishi[7] = 4;
    m_qishi[8] = 6; m_qishi[9] = 8; 
}
...
bool CCalender::setDateTime(const CDateTime& datetime) {
    ...
    m_bigMon = m_isLeap ? getLeapDays(m_ldatetime.m_date.m_nYear) != 0:
               getLunarDay(m_ldatetime.m_date.m_nYear, m_ldatetime.m_date.m_nMon) > 29;
...
// 返回距离 00:00:00 的秒数
int CCalender::getSecondsFromBase(const CTime& time) {
    
    return time.m_nHour * 3600 + time.m_nMin * 60 + time.m_nSec;
}

int CCalender::getDaysFromBase(const CDate& date) {
...
bool CCalender::isLeap(int nYear) {
...
// 返回下一个月的年干支月干支信息
void CCalender::getNextJie(int& nYear, int& nMon, int& nJie, int& nMonJiaZi, int& nYearJiaZi) {
...
void CCalender::getNextYMGanZhi(const int nJiaZiY, int& nJiaZiNextY, int& nJiaZiNextM) {
...
void CCalender::getPreMonYMGanZhi(const int nJiaZiY, const int nJiaZiM, int& nJiaZiYOut, int& nJiaZiMOut, bool isLiChun) {
...
// 返回两个日期之间的天数差
int CCalender::getDiffByTwoDate(const CDate& dateA, const CDate& dateB) {
...
// 返回两个时间之间的秒数差
int CCalender::getDiffByTwoTime(const CTime& timeA, const CTime& timeB) {
...
// 返回两个日期时间的秒数差
long long CCalender::getSecondByTwoDateTime(const CDateTime& datetimeA, const CDateTime& datetimeB) {
...
void CCalender::getNextDay(CDateTime& datetime) {
    ...
    getNextDay(datetime, datetimeTem);
...
void CCalender::getNextDay(CDate& date) {
    ...
    getNextDay(datetimeTemBase, datetimeOut);
    date = datetimeOut.m_date;
}
// 返回公历日期的后一天日期
void CCalender::getNextDay(const CDateTime& datetime, CDateTime& outtime) {
...
void CCalender::getDateTimeBySecond(const CDateTime& basetime, CDateTime& outtime, long long nSecond) {
