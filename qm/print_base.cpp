...
void CCmdPrint::PrintBase()
{
    ...
    std::cout << "公元：" << COLOR_CYAN << solar.m_date.m_nYear << COLOR_RESET << "年";
    std::cout << COLOR_CYAN << std::setw(2) << std::setfill('0') << solar.m_date.m_nMon << COLOR_RESET << "月";
    std::cout << COLOR_CYAN << std::setw(2) << std::setfill('0') << solar.m_date.m_nDay << COLOR_RESET << "日 ";
    std::cout << COLOR_CYAN << std::setw(2) << std::setfill('0') << solar.m_time.m_nHour << COLOR_RESET << "时";
    std::cout << COLOR_CYAN << std::setw(2) << std::setfill('0') << solar.m_time.m_nMin << COLOR_RESET << "分";
    std::cout << COLOR_CYAN << std::setw(2) << std::setfill('0') << solar.m_time.m_nSec << COLOR_RESET << "秒       ";
    ...
    std::cout << COLOR_CYAN << CZhData::ZhMon(lunar.m_date.m_nMon - 1) << COLOR_RESET << "月";
    std::cout << COLOR_CYAN << CZhData::ZhLunarDay(lunar.m_date.m_nDay - 1) << COLOR_RESET << "日 ";
    std::cout << COLOR_CYAN << CZhData::ZhZhi((lunar.m_time.m_nHour + 1) / 2 % 12) << COLOR_RESET << "时\n";
    ...
    std::cout << "值符：" << COLOR_YELLOW << CZhData::ZhJiuXing(m_qm->getJiuXingPre()[m_qm->getDuty()]) << COLOR_RESET;
    std::cout << "  值使：" << COLOR_YELLOW << CZhData::ZhBaMen(m_qm->getBaMenPre()[m_qm->getDuty()]) << " ";
    ...
    std::string jieQi = CZhData::ZhJieQi(m_qm->getCalendar()->getJieFirst().index);
    if (m_qm->getYuan() == 0) {
        szYuan = "   [" + szYuan + "]";
    }
    else {
...
void CCmdPrint::PrintOther()
{
    ...
    szXunkong += CZhData::ZhZhi(m_qm->getXunKong()[0]) + CZhData::ZhZhi(m_qm->getXunKong()[1]) + "   ";
    szXunkong += CZhData::ZhZhi(m_qm->getXunKong()[2]) + CZhData::ZhZhi(m_qm->getXunKong()[3]) + "   ";
    szXunkong += CZhData::ZhZhi(m_qm->getXunKong()[4]) + CZhData::ZhZhi(m_qm->getXunKong()[5]) + "   ";
    szXunkong += CZhData::ZhZhi(m_qm->getXunKong()[6]) + CZhData::ZhZhi(m_qm->getXunKong()[7]) + "   \n";
    std::cout << COLOR_YELLOW << szXunkong << COLOR_RESET;
    std::cout << COLOR_GREEN << SPLIT_LINE << COLOR_RESET << std::endl;

    std::string szJieName;
    szJieName += "[" + CZhData::ZhJieQi(m_qm->getCalendar()->getJieFirst().index) + "]：";
    std::cout << szJieName;
    char szTem[128]{};
    ...
    szJieName.clear();
    szJieName += "  [" + CZhData::ZhJieQi(m_qm->getCalendar()->getJieSecond().index) + "]：";
    std::cout << szJieName;
    std::snprintf(szTem, sizeof(szTem), "%02d-%02d %02d:%02d:%02d",
